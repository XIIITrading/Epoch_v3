...
j_m1_trade_indicator - CLI Runner
XIII Trading LLC
================================================================================

Command-line interface for the entry-bar indicator snapshot populator.
...
from db_config import DB_CONFIG, SCHEMA_DIR, J_M1_TRADE_INDICATOR_TABLE
from populator import JM1TradeIndicatorPopulator
...
def run_schema():
    """Execute the schema SQL to create the j_m1_trade_indicator table."""
    import psycopg2

    schema_file = SCHEMA_DIR / "j_m1_trade_indicator.sql"
    ...
    print("=" * 60)
    print("j_m1_trade_indicator - Schema Creation")
    print("=" * 60)
...
def show_status():
    ...
    print()
    print("=" * 60)
    print("J_M1 TRADE INDICATOR - Processing Status")
    print("=" * 60)

    try:
        conn = psycopg2.connect(**DB_CONFIG)
        populator = JM1TradeIndicatorPopulator(verbose=False)
...
def run_population(args):
    """Run the j_m1_trade_indicator population."""
    print()
    print("=" * 60)
    print("EPOCH JOURNAL SYSTEM")
    print("j_m1_trade_indicator Populator v1.0.0")
    print("=" * 60)
    print()
    ...
    populator = JM1TradeIndicatorPopulator(verbose=args.verbose)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Populate j_m1_trade_indicator with entry-bar indicator snapshots',
...
Output:
  Results are written to the j_m1_trade_indicator table in Supabase.
  Each trade gets 1 row with trade context + outcome + indicator snapshot.
        """
    )
    ...
    parser.add_argument(
        '--schema',
        action='store_true',
        help='Run schema creation only (create j_m1_trade_indicator table)'
    )
