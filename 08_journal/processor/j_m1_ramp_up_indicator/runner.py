...
j_m1_ramp_up_indicator - CLI Runner
XIII Trading LLC
================================================================================

Command-line interface for the ramp-up indicator bars populator.
...
from db_config import DB_CONFIG, SCHEMA_DIR, J_M1_RAMP_UP_INDICATOR_TABLE, RAMP_UP_BARS
from populator import JM1RampUpIndicatorPopulator
...
def run_schema():
    """Execute the schema SQL to create the j_m1_ramp_up_indicator table."""
    import psycopg2

    schema_file = SCHEMA_DIR / "j_m1_ramp_up_indicator.sql"
    ...
    print("=" * 60)
    print("j_m1_ramp_up_indicator - Schema Creation")
    print("=" * 60)
...
def show_status():
    ...
    print()
    print("=" * 60)
    print("J_M1 RAMP-UP INDICATOR - Processing Status")
    print("=" * 60)

    try:
        conn = psycopg2.connect(**DB_CONFIG)
        populator = JM1RampUpIndicatorPopulator(verbose=False)
...
def run_population(args):
    """Run the j_m1_ramp_up_indicator population."""
    print()
    print("=" * 60)
    print("EPOCH JOURNAL SYSTEM")
    print("j_m1_ramp_up_indicator Populator v1.0.0")
    print("=" * 60)
    print()
    print(f"Start Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"Mode: {'DRY-RUN' if args.dry_run else 'FULL RUN'}")
    print(f"Ramp-Up Bars: {RAMP_UP_BARS}")
    if args.limit:
        print(f"Limit: {args.limit} trades")
    print()

    populator = JM1RampUpIndicatorPopulator(verbose=args.verbose)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Populate j_m1_ramp_up_indicator with 25 M1 bars before entry',
...
Output:
  Results are written to the j_m1_ramp_up_indicator table in Supabase.
  Each trade gets up to 25 rows (bar_sequence 0-24), one per M1 bar.
    ...
    parser.add_argument(
        '--schema',
        action='store_true',
        help='Run schema creation only (create j_m1_ramp_up_indicator table)'
    )
