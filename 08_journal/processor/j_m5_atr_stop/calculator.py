...
J_M5 ATR Stop Calculator
XIII Trading LLC
================================================================================

Evaluates journal trades using M5 ATR(14) as the stop distance (1R) and tracks
...
still walks M1 bars from j_m1_bars for maximum detection fidelity.

LOGIC:
    1. Read entry from journal_trades, adjust entry_time to M1 candle (truncate seconds)
    2. Fetch pre-computed atr_m5 from j_m1_indicator_bars at adjusted entry candle
...
DATA SOURCES (journal tables):
    ...
    - j_m1_indicator_bars (pre-computed atr_m5 at entry candle)
...
from db_config import (
    ...
    J_M5_ATR_STOP_TABLE, EOD_CUTOFF, R_LEVELS, BATCH_INSERT_SIZE,
...
@dataclass
class JM5AtrStopResult:
    """Result of M5 ATR Stop calculation for a single journal trade."""
...
class JM5AtrStopCalculator:
    ...
    def get_trades_needing_calculation(self, conn, limit: int = None) -> List[Dict[str, Any]]:
        """
        Query journal_trades for trades not yet processed in j_m5_atr_stop.
        ...
        query = f"""
            ...
            WHERE t.entry_time IS NOT NULL
              ...
              AND NOT EXISTS (
                  SELECT 1 FROM {J_M5_ATR_STOP_TABLE} r
    ...
    def get_m5_atr_at_entry(
        ...
        """
        Fetch pre-computed atr_m5 from j_m1_indicator_bars at the adjusted entry candle.
    ...
    def insert_results(self, conn, results: List[JM5AtrStopResult]) -> int:
        """Insert calculation results into j_m5_atr_stop table."""
        if not results:
            return 0

        query = f"""
            INSERT INTO {J_M5_ATR_STOP_TABLE} (
    ...
    def run_batch_calculation(
        ...
        print("=" * 60)
        print("J_M5 ATR Stop Calculator")
        print("=" * 60)
        print(f"Stop Type: M5 ATR(14) x 1.0")
...
if __name__ == "__main__":
    print("J_M5 ATR Stop Calculator - Test Mode")
    print("=" * 60)

    calculator = JM5AtrStopCalculator(verbose=True)
