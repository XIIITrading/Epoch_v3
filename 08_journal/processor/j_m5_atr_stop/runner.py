...
J_M5 ATR Stop Processor - CLI Runner
XIII Trading LLC
================================================================================

Command-line interface for running the J_M5 ATR Stop calculation.
...
from db_config import DB_CONFIG, SCHEMA_DIR, R_LEVELS, EOD_CUTOFF, SOURCE_TABLE, J_M5_ATR_STOP_TABLE
from calculator import JM5AtrStopCalculator
...
def run_schema():
    """Execute the schema SQL to create the j_m5_atr_stop table."""
    import psycopg2

    schema_file = SCHEMA_DIR / "j_m5_atr_stop.sql"
    ...
    print("=" * 60)
    print("J_M5 ATR Stop - Schema Creation")
    print("=" * 60)
...
def show_status():
    ...
    print()
    print("=" * 60)
    print("J_M5 ATR STOP - Processing Status")
    print("=" * 60)
...
def run_calculation(args):
    """Run the J_M5 ATR Stop calculation."""
    print()
    print("=" * 60)
    print("EPOCH TRADING SYSTEM")
    print("J_M5 ATR Stop Calculator v1.0.0")
    print("=" * 60)
    print()
    ...
    # Create calculator
    calculator = JM5AtrStopCalculator(verbose=args.verbose)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Calculate M5 ATR Stop outcomes for journal trades using M5 ATR(14) stop and R-multiple targets (1R-5R)',
...
Output:
  Results are written to the j_m5_atr_stop table in Supabase.
  Each trade gets 1 row with R-level hit tracking and outcome.
    ...
    parser.add_argument(
        '--schema',
        action='store_true',
        help='Run schema creation only (create j_m5_atr_stop table)'
    )
