...
j_m1_post_trade_indicator - Populator
XIII Trading LLC
================================================================================

Captures the 25 M1 indicator bars AFTER entry for each journal trade.
bar_sequence 0 = entry candle, bar_sequence 24 = 25th bar after entry.

Outcome (is_winner, pnl_r, max_r_achieved) is stamped on EVERY row
for easy aggregation and filtering.
...
from db_config import (
    ...
    J_M1_POST_TRADE_INDICATOR_TABLE, J_M5_ATR_STOP_TABLE,
    BATCH_INSERT_SIZE, VERBOSE, POST_TRADE_BARS,
...
class JM1PostTradeIndicatorPopulator:
    """
    Populates j_m1_post_trade_indicator with 25 M1 bars after each trade entry.

    For each journal trade that has a j_m5_atr_stop outcome:
    1. Fetches 25 M1 indicator bars starting at entry candle (>= entry_m1)
    2. Assigns bar_sequence 0 (entry candle) through 24
    3. Stamps outcome (is_winner, pnl_r, max_r_achieved) on EVERY row
    4. Inserts one row per bar with trade_id + bar_sequence as composite key
    """
    ...
    def get_eligible_trades(self, conn, limit: int = None) -> List[Dict[str, Any]]:
        ...
        but are NOT yet in j_m1_post_trade_indicator.
        """
        query = f"""
            ...
            WHERE NOT EXISTS (
                SELECT 1 FROM {J_M1_POST_TRADE_INDICATOR_TABLE} pt
    ...
    def get_post_trade_bars(
        self, conn, ticker: str, trade_date, entry_time: time
    ) -> List[Dict[str, Any]]:
        """
        Fetch up to 25 M1 indicator bars STARTING at entry candle.

        bar_sequence 0 = entry candle, bar_sequence 24 = 25th bar after.
        Returns bars in chronological order (ascending time).
        """
    ...
    def build_rows(
        ...
        """
        Build list of dicts for j_m1_post_trade_indicator rows.
        One row per bar, bar_sequence 0 = entry candle, N-1 = last bar.
        STAMPS OUTCOME on every row.
        """
        # Outcome values (stamped on every row)
        ...
        for seq, bar in enumerate(indicator_bars):
            ...
            row['pnl_r'] = pnl_r
            row['max_r_achieved'] = max_r_achieved
    ...
    def insert_results(self, conn, rows: List[Dict[str, Any]]) -> int:
        """Insert results into j_m1_post_trade_indicator with ON CONFLICT DO NOTHING."""
        if not rows:
            return 0

        query = f"""
            INSERT INTO {J_M1_POST_TRADE_INDICATOR_TABLE} (
    ...
    def get_status(self, conn) -> Dict[str, Any]:
        ...
        with conn.cursor() as cur:
            ...
            cur.execute(f"SELECT COUNT(DISTINCT trade_id) FROM {J_M1_POST_TRADE_INDICATOR_TABLE}")
            status['trades_processed'] = cur.fetchone()[0]

            # Total rows
            cur.execute(f"SELECT COUNT(*) FROM {J_M1_POST_TRADE_INDICATOR_TABLE}")
            ...
            # Win/loss breakdown
            cur.execute(f"""
                SELECT is_winner, COUNT(DISTINCT trade_id)
                FROM {J_M1_POST_TRADE_INDICATOR_TABLE}
                GROUP BY is_winner
            """)
    ...
    def run_batch_population(
        ...
        """
        Main entry point. Process all trades needing post-trade indicator bars.
        ...
        _emit("=" * 60)
        _emit("Journal M1 Post-Trade Indicator Populator v1.0")
        _emit("=" * 60)
        _emit(f"Source: {SOURCE_TABLE} + {J_M5_ATR_STOP_TABLE} + {J_M1_INDICATOR_BARS_TABLE}")
        _emit(f"Target: {J_M1_POST_TRADE_INDICATOR_TABLE}")
        _emit(f"Post-Trade Bars: {POST_TRADE_BARS}")
        ...
        try:
            ...
            for idx, trade in enumerate(trades, 1):
                ...
                try:
                    ...
                    # Get post-trade bars
                    post_bars = self.get_post_trade_bars(conn, ticker, trade_date, entry_time)

                    if not post_bars:
                        self._log(
                            f"Skipping {trade_id}: no post-trade bars "
                    ...
                    # Build rows
                    rows = self.build_rows(trade, post_bars)
                    ...
                    _emit(
                        ...
                        f"{winner_str:<5s} bars={len(post_bars)}"
                    )
...
if __name__ == "__main__":
    print("J_M1 Post-Trade Indicator Populator - Test Mode")
    print("=" * 60)

    populator = JM1PostTradeIndicatorPopulator(verbose=True)
