"""
Epoch Trading Journal - Ramp-Up Chart Component
Displays M1 candlestick chart with vertical indicator table below each bar.
Adapted from 06_training/components/rampup_chart.py.

Data is fetched from pre-computed journal_m1_indicator_bars table.
Calculations are performed by 08_journal/processor/populator.py.

7-row indicator display:
    1. Candle %    - candle_range_pct
    2. Vol Delta   - vol_delta
    3. Vol ROC     - vol_roc
    4. SMA         - sma config + spread
    5. H1 Struct   - h1_structure
    6. LONG        - long_score (0-7)
    7. SHORT       - short_score (0-7)
"""
...
def fetch_m1_bars_for_rampup(
    ...
    """
    Fetch M1 indicator bars from journal_m1_indicator_bars for ramp-up chart.
    ...
    Returns:
        DataFrame with M1 bars and indicators, chronological order
    """
    query = """
        SELECT
        ...
        FROM journal_m1_indicator_bars
        WHERE ticker = %s
          AND bar_date = %s
          AND bar_time < %s
    ...
    try:
        ...
        print(f"Error fetching M1 bars for rampup: {e}")
        return pd.DataFrame()
...
def get_vol_delta_color(vol_delta) -> str:
    ...
    if vol_delta is None:
        return COLORS['default']
    return COLORS['vol_delta_positive'] if vol_delta > 0 else COLORS['vol_delta_negative']
...
def get_sma_config_color(sma_spread) -> str:
    ...
    if sma_spread is None:
        return COLORS['default']
    return COLORS['sma_bull'] if sma_spread > 0 else COLORS['sma_bear']
...
def format_sma_config(sma_spread, close) -> str:
    ...
    if sma_spread is None or close is None or close == 0:
        return '-'
    config = 'B' if sma_spread > 0 else 'S'
...
def build_rampup_chart(
    ...
    if df.empty:
        fig = go.Figure()
        fig.add_annotation(
            text="No M1 indicator data available. Run: python -m processor.populator",
    ...
    # ==========================================================================
    # ROW 2: INDICATOR TABLE (colored text annotations)
    # ==========================================================================
    ...
    for indicator_idx, indicator_name in enumerate(INDICATOR_LABELS):
        ...
        for bar_idx, (_, row) in enumerate(df.iterrows()):
            if indicator_name == 'Candle %':
                ...
                value = format_sma_config(row.get('sma_spread'), row.get('close'))
                text_color = get_sma_config_color(row.get('sma_spread'))
...
# =============================================================================
# MAIN RENDER FUNCTION (called from review page)
# =============================================================================
