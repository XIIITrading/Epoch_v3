"""
Epoch Trading System - M1 Post-Trade Indicator Table (Journal Viewer)
QTableWidget showing 7 indicator rows x N bar columns, from entry onward.
Entry candle is the first column (labeled "E"), followed by subsequent bars.
Data sourced from j_m1_post_trade_indicator table via JournalTradeLoader.

Adapted from 11_trade_reel/ui/posttrade_table.py.
...
# Entry candle + 24 candles after = 25 total columns
POSTTRADE_BARS = 25
...
def fetch_posttrade_data(
    ...
    """Fetch M1 post-trade indicator bars from j_m1_post_trade_indicator via loader."""
    try:
        from data.journal_loader import get_journal_loader
        loader = get_journal_loader()
        df = loader.fetch_posttrade_data(ticker, trade_date, entry_time)
        return df
    except ImportError:
        logger.warning("JournalTradeLoader not available, falling back to direct DB query")
        return _fetch_posttrade_data_direct(ticker, trade_date, entry_time, num_bars)
    except Exception as e:
        logger.error(f"Error fetching post-trade data: {e}")
        return pd.DataFrame()


def _fetch_posttrade_data_direct(
    ticker: str,
    trade_date: date,
    entry_time: time,
    num_bars: int = POSTTRADE_BARS,
) -> pd.DataFrame:
    """Fallback: fetch M1 post-trade indicator bars directly from DB."""
    import psycopg2
    ...
    query = """
        SELECT
            ...
            candle_range_pct
        FROM j_m1_post_trade_indicator
        WHERE ticker = %s
          AND bar_date = %s
          AND bar_time >= %s
    ...
    try:
        ...
        logger.error(f"Error fetching post-trade data (direct): {e}")
        return pd.DataFrame()
...
class PostTradeTable(QFrame):
    ...
    def update_data(self, df: pd.DataFrame):
        ...
        # Column headers = time labels; first column is entry (labeled "E")
        time_labels = []
        for i, (_, row) in enumerate(df.iterrows()):
            ...
            # Mark the entry candle with "E" prefix
            if i == 0:
                label = f"E {label}"
