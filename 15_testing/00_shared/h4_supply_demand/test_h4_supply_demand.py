"""
================================================================================
EPOCH TRADING SYSTEM - H4 Supply & Demand Zone Test
Interactive terminal test for H4 pivot-based supply/demand zones.
XIII Trading LLC
================================================================================

Usage:
    python 15_testing/00_shared/h4_supply_demand/test_h4_supply_demand.py
    python 15_testing/00_shared/h4_supply_demand/test_h4_supply_demand.py MU 2026-03-17

Description:
    Prompts for ticker and date, then:
    a) Fetches ~180 calendar days of H4 bars ending at 08:00 ET on analysis date
    b) Calculates D1 ATR (14-period, same as PDV)
    c) Gets price at 08:00 ET
    d) Runs H4 S/D zone detection with 3-ATR filter from 08:00 price
...
from shared.calculations.h4_supply_demand import (
    calculate_h4_zones,
    H4SupplyDemandResult,
    Zone,
    ZoneType,
)
from shared.calculations.h4_supply_demand.calculator import MAX_FLIPS, MAX_TOUCHES

ET_TIMEZONE = ZoneInfo("America/New_York")
H4_LOOKBACK_DAYS = 180   # calendar days (~120 trading days of H4 bars)
...
def _fetch_h4_bars(client, ticker: str, analysis_date: date) -> pd.DataFrame:
    """Fetch H4 bars ending at 08:00 ET on analysis date."""
    end_ts = datetime(
        analysis_date.year, analysis_date.month, analysis_date.day,
        8, 0, 0, tzinfo=ET_TIMEZONE,
    )
    start_date = analysis_date - timedelta(days=H4_LOOKBACK_DAYS)

    df = client.fetch_4h_bars(ticker, start_date, end_timestamp=end_ts)
    if df is None or df.empty:
        return pd.DataFrame()
...
def display_result(
    ...
    print(f"  {BOLD}  H4 Supply & Demand Zones - {result.ticker} ({analysis_date}){RESET}")
    print(f"  {BOLD}{sep}{RESET}")
    print()
    ...
    print(f"  {DIM}H4 Bars:{RESET}          {result.bar_count}")
    print(
...
def run(ticker: str, analysis_date: date):
    """Run the H4 S/D calculation and display results."""
    ticker = ticker.upper()

    print(f"\n  Calculating H4 Supply & Demand for {ticker} on {analysis_date}...")
    print(f"  (Fetching ~{H4_LOOKBACK_DAYS} days of H4 bars + D1 ATR)")
    print()
    ...
    # Step 3: H4 bars
    print(f"  {DIM}  Fetching H4 bars...{RESET}")
    df_h4 = _fetch_h4_bars(client, ticker, analysis_date)

    if df_h4.empty:
        print(f"  {BEAR_COLOR}ERROR: No H4 data returned for {ticker}{RESET}")
        return
...
def main():
    parser = argparse.ArgumentParser(
        description="Test H4 Supply & Demand zone calculation",
    )
    parser.add_argument(
