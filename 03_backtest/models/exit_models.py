...
class M5StructureTracker:
    ...
    def update(self, high: float, low: float, close: float) -> tuple:
        """Update structure with new bar. Returns: (choch_bullish, choch_bearish)"""
...
class ExitManager:
    ...
    def check_exit(self, bar_idx: int, bar_time: datetime,
                   bar_open: float, bar_high: float,
                   bar_low: float, bar_close: float,
                   is_long: bool, entry_price: float,
                   stop_price: float, target_used: float,
                   target_type: str, risk: float) -> Optional[ExitSignal]:
        ...
        if self.use_choch and self.structure_tracker:
            ...
            if is_long and choch_bear:
                return self._create_exit(
                    ExitReason.CHOCH, bar_close, bar_time, bar_idx,
                    entry_price, risk, is_long
                )
            elif not is_long and choch_bull:
                return self._create_exit(
                    ExitReason.CHOCH, bar_close, bar_time, bar_idx,
                    entry_price, risk, is_long
                )
