...
class OptimalTradeCalculator:
    ...
    def get_trades_with_mfe_mae(self, conn, limit: Optional[int] = None) -> List[TradeData]:
        """
        Get trades that have mfe_mae_potential data but NOT yet in optimal_trade.

        Returns trades joined with mfe_mae_potential, excluding already processed.
        Uses LEFT JOIN to find trades needing calculation (incremental processing).
        """
        query = f"""
            SELECT
                t.trade_id,
            ...
            LEFT JOIN {TARGET_TABLE} ot ON t.trade_id = ot.trade_id
    ...
    def truncate_optimal_trade(self, conn):
        """Delete all rows from optimal_trade table (used only with --rebuild flag)."""
        with conn.cursor() as cur:
    ...
    def run_calculation(
        ...
        """
        Main entry point. Process trades incrementally (or rebuild all).

        Args:
            limit: Max trades to process (for testing)
            dry_run: If True, calculate but don't write to DB
            rebuild: If True, truncate table and rebuild all (default: False for incremental)
        ...
        print("=" * 70)
        print("OPTIMAL TRADE CALCULATOR (Points-Based) v2.1.0")
        print("=" * 70)
        print(f"EOD Cutoff: {EOD_CUTOFF}")
        print(f"Mode: {'REBUILD (full)' if rebuild else 'INCREMENTAL'}")
        ...
        try:
            ...
            # Only truncate if rebuild mode
            if rebuild:
                print("\n[2/4] Truncating optimal_trade table (rebuild mode)...")
                if not dry_run:
                    self.truncate_optimal_trade(conn)
                else:
                    print("  [DRY-RUN] Would truncate optimal_trade table")
            else:
                print("\n[2/4] Incremental mode - skipping truncate")

            # Get trades with MFE/MAE data (query filters out already-processed trades)
            print("\n[3/4] Querying trades needing calculation...")
            trades = self.get_trades_with_mfe_mae(conn, limit)
            print(f"  Found {len(trades)} trades to process")

            if not trades:
                print("\n  All trades already processed. Nothing to do.")
