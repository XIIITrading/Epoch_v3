...
from eq_config import PREFETCH_BARS, VOL_DELTA_ROLL_PERIOD, VOL_ROC_LOOKBACK, H1_BARS_NEEDED


# Global H1 structure cache (shared across workers)
...
class DataWorker(QThread):
    ...
    def _fetch_and_process(self, ticker: str):
        ...
        # Fetch/use cached H1 bars and calculate structure
        h1_results = self._get_h1_structure(ticker, bars)

        # Combine bar data with calculations
        processed_bars = []
        for i, (bar, delta, candle_range, vol_roc, sma, h1) in enumerate(
            zip(bars, delta_results, range_results, vol_roc_results, sma_results, h1_results)
        ):
