...
def format_date_for_api(date: Union[str, datetime, pd.Timestamp]) -> str:
    """
    Format date for Polygon API (YYYY-MM-DD or Unix timestamp for intraday)
    """
    # Parse to datetime first
    dt = parse_date(date)

    # For ANY intraday data (including seconds), use Unix timestamp in milliseconds
    # Check if this is a datetime with time component (not just midnight)
    if isinstance(date, str) and ' ' in date:
        # Has time component - use Unix timestamp
        timestamp_ms = int(dt.timestamp() * 1000)
        return str(timestamp_ms)
    elif dt.hour != 0 or dt.minute != 0 or dt.second != 0:
        # Has non-midnight time - use Unix timestamp
        timestamp_ms = int(dt.timestamp() * 1000)
        return str(timestamp_ms)
    else:
...
def split_large_date_range(start_date: Union[str, datetime],
                           end_date: Union[str, datetime],
                           max_days: Union[int, float]) -> List[Tuple[datetime, datetime]]:
    """
    [FUNCTION SUMMARY]
    ...
    Parameters:
        - start_date: Start date
        - end_date: End date
        - max_days: Maximum days per chunk (can be float for sub-day chunks)
    ...
    # Handle sub-day chunks (for second/minute data)
    if max_days < 1:
        # Convert to seconds for more precise splitting
        ...
        while current_start < end_dt:
            # Calculate chunk end
            chunk_end = min(
                current_start + timedelta(seconds=max_seconds),
                end_dt
            )
    ...
    while current_start < end_dt:
        # Calculate chunk end
        chunk_end = min(
            current_start + timedelta(days=int(max_days)),
            end_dt
        )
...
def normalize_ohlcv_data(data: List[Dict[str, Any]]) -> pd.DataFrame:
    ...
    column_mapping = {
        ...
        'c': 'close',
        'v': 'volume',
        't': 'timestamp',  # Some responses use 't'
        's': 'start_timestamp',  # Start timestamp in milliseconds
        'e': 'end_timestamp',  # End timestamp in milliseconds
        'vw': 'vwap',  # Volume weighted average price
        'n': 'transactions',  # Number of transactions
        'a': 'average',  # Average price
        'op': 'official_open',  # Official open price
        'av': 'aggregate_volume',  # Aggregate volume
        'z': 'average_trade_size'  # Average trade size
    }

    df = df.rename(columns=column_mapping)

    # Determine which timestamp to use for the index
    # Priority: end_timestamp (e) > timestamp (t) > start_timestamp (s)
    timestamp_col = None
    ...
    if timestamp_col:
        ...
        # Keep both start and end timestamps if available for reference
        if 'start_timestamp' in df.columns and 'end_timestamp' in df.columns:
            # Convert to datetime for easier use
            df['bar_start'] = pd.to_datetime(df['start_timestamp'], unit='ms', utc=True)
            df['bar_end'] = pd.to_datetime(df['end_timestamp'], unit='ms', utc=True)

            # Calculate bar duration in seconds (useful for second bars)
            df['bar_duration_ms'] = df['end_timestamp'] - df['start_timestamp']
            df['bar_duration_sec'] = df['bar_duration_ms'] / 1000.0

        # Drop the raw timestamp columns
        cols_to_drop = [col for col in ['timestamp', 'start_timestamp', 'end_timestamp'] if col in df.columns]
        df = df.drop(columns=cols_to_drop)

    # Ensure numeric types
    numeric_columns = ['open', 'high', 'low', 'close', 'volume', 'vwap', 'transactions',
                       'average', 'official_open', 'aggregate_volume', 'average_trade_size',
                       'bar_duration_ms', 'bar_duration_sec']
