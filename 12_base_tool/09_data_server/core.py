...
class PolygonClient:
    ...
    def get_aggregates(self, ticker: str, multiplier: int, timespan: str,
        ...
        Parameters:
            ...
            - timespan (str): Time unit (second, minute, hour, day, etc.)
            - from_date (str): Start date (YYYY-MM-DD or YYYY-MM-DD HH:MM:SS for seconds)
            - to_date (str): End date (YYYY-MM-DD or YYYY-MM-DD HH:MM:SS for seconds)
            - adjusted (bool): Include adjusted prices
            - sort (str): Sort order (asc/desc)
            - limit (int): Maximum results
        Returns: dict - API response with results
        Example: data = client.get_aggregates('AAPL', 1, 'second', '2023-01-01 09:30:00', '2023-01-01 09:31:00')
        """
        # Validate timespan
        valid_timespans = ['second', 'minute', 'hour', 'day', 'week', 'month', 'quarter', 'year']
        if timespan not in valid_timespans:
            raise ValueError(f"Invalid timespan: {timespan}. Must be one of {valid_timespans}")

        # Apply appropriate limits based on timespan
        if timespan == 'second':
            # Polygon has stricter limits for second data
            if limit > 5000:
                self.logger.warning(f"Reducing limit from {limit} to 5000 for second data")
                limit = 5000

            # Ensure dates include time for second data
            if len(from_date) == 10:  # Just date, no time
                from_date = f"{from_date} 00:00:00"
            if len(to_date) == 10:  # Just date, no time
                to_date = f"{to_date} 23:59:59"

            self.logger.debug(f"Fetching {multiplier}-second bars from {from_date} to {to_date}")
        ...
        if 'results' not in response:
            ...
            self.logger.info(f"Received {len(response.get('results', []))} {timespan} bars for {ticker}")
