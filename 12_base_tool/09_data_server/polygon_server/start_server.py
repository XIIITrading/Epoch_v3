...
try:
    ...
    if env_path.exists():
        ...
        print("No .env file found and python-dotenv not available")
...
def configure_environment():
    ...
    if not polygon_key:
        print(f"{Colors.RED}[ERROR] POLYGON_API_KEY not found{Colors.END}")
        ...
        if not env_file.exists():
            ...
            create_env_template()
            print(f"{Colors.GREEN}[OK] Created .env file. Please add your API key and restart.{Colors.END}")
            sys.exit(1)
        ...
        print(f"{Colors.GREEN}[OK] Polygon API key found: {masked_key}{Colors.END}")
    ...
    print(f"{Colors.GREEN}[OK] Environment configured{Colors.END}")
...
def install_dependencies():
    ...
    if requirements_file.exists():
        ...
        print(f"{Colors.GREEN}[OK] Dependencies installed{Colors.END}")
    else:
...
def start_server(host: str = None, port: int = None, workers: int = 1, reload: bool = False):
    ...
    # Set up paths for proper module resolution
    data_server_dir = project_root / '09_data_server'
    if str(data_server_dir) not in sys.path:
        sys.path.insert(0, str(data_server_dir))
    
    print(f"{Colors.GREEN}[OK] Environment configured for server startup{Colors.END}")
    ...
    try:
        # Change to 09_data_server directory for proper module resolution
        os.chdir(data_server_dir)
        
        # Use the module path that works from 09_data_server directory
        module_path = 'polygon_server.server:app'
        ...
        try:
            ...
            print(f"{Colors.RED}[ERROR] uvicorn not installed{Colors.END}")
            print(f"Install with: {sys.executable} -m pip install uvicorn[standard]")
            sys.exit(1)
        except Exception as e:
            print(f"{Colors.RED}[ERROR] Server startup failed: {e}{Colors.END}")
            sys.exit(1)
    except KeyboardInterrupt:
        print(f"\n{Colors.YELLOW}Shutting down server...{Colors.END}")
        print(f"{Colors.GREEN}[OK] Server stopped{Colors.END}")
...
def test_connection(port: int = 8200):
    ...
    try:
        ...
        if response.status_code == 200:
            print(f"{Colors.GREEN}[OK] Server is running and healthy{Colors.END}")
            ...
            print(f"{Colors.RED}[ERROR] Server returned status {response.status_code}{Colors.END}")
            return False
    except requests.exceptions.ConnectionError:
        print(f"{Colors.RED}[ERROR] Could not connect to server at port {port}{Colors.END}")
