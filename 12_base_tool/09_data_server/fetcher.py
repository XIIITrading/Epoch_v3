...
class DataFetcher:
    ...
    def _fetch_from_api(self, symbol: str, multiplier: int, timespan: str,
                        ...
                        adjust_splits: bool = True) -> pd.DataFrame:
        """
        Fetch data from Polygon API with rate limiting
        Parameters:
            - symbol (str): Stock symbol
        ...
        # For second-level data, use much smaller chunks
        if timespan == 'second':
            # Polygon limits second data to smaller ranges
            # Maximum 1 hour of second data per request
            max_seconds = 3600  # 1 hour
            total_seconds = (end_date - start_date).total_seconds()

            if total_seconds > max_seconds:
                # Split into hour chunks for second data
                date_ranges = []
                current_start = start_date
                while current_start < end_date:
                    chunk_end = min(current_start + timedelta(seconds=max_seconds), end_date)
                    date_ranges.append((current_start, chunk_end))
                    current_start = chunk_end + timedelta(seconds=1)
            else:
                date_ranges = [(start_date, end_date)]
        else:
            # For other timespans, use normal day-based splitting
        ...
        for i, (chunk_start, chunk_end) in enumerate(date_ranges):
            ...
            try:
                ...
                # Adjust limit for second data
                api_limit = 50000
                if timespan == 'second':
                    api_limit = 5000  # Smaller limit for second data
                ...
                if response.get('results'):
                    ...
                    if not df_chunk.empty:
                        ...
                        self.logger.debug(
                            ...
                            f"{chunk_start.strftime('%Y-%m-%d %H:%M:%S')} to {chunk_end.strftime('%Y-%m-%d %H:%M:%S')}"
                        )
                else:
                    self.logger.warning(
