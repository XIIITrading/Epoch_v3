...
M5 Indicator Bars - CLI Runner
XIII Trading LLC
================================================================================

Command-line interface for running the M5 Indicator Bars population.
...
def run_schema():
    """Execute the schema SQL to create the m5_indicator_bars table."""
    import psycopg2

    schema_file = SCHEMA_DIR / "m5_indicator_bars.sql"
    ...
    print("=" * 60)
    print("M5 Indicator Bars - Schema Creation")
    print("=" * 60)
...
def run_status():
    """Show current status of the m5_indicator_bars table."""
    import psycopg2

    print("=" * 60)
    print("M5 Indicator Bars - Status")
    print("=" * 60)
    ...
    try:
        ...
        populator = M5IndicatorBarsPopulator(verbose=False)
...
def run_population(args):
    """Run the M5 indicator bars population."""
    print()
    print("=" * 60)
    print("EPOCH TRADING SYSTEM")
    print("M5 Indicator Bars Calculator v1.0.0")
    print("=" * 60)
    print()
    ...
    populator = M5IndicatorBarsPopulator(verbose=args.verbose)
...
Output:
  Results are written to the m5_indicator_bars table in Supabase.
  Each ticker-date produces ~78 rows (one per M5 bar in trading day).
        """
    )
    ...
    parser.add_argument(
        '--schema',
        action='store_true',
        help='Run schema creation only (create m5_indicator_bars table)'
    )

    parser.add_argument(
        '--status',
        action='store_true',
        help='Show current status of m5_indicator_bars table'
    )
