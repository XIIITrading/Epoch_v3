...
Entry Indicators - Market Structure Detection
XIII Trading LLC
================================================================================

Self-contained market structure calculation based on ticker_structure_calculator.
Detects fractals, identifies BOS/ChoCH, and determines market direction.

Supports M5, M15, H1, and H4 timeframes for structure detection.
...
class HTFBarFetcher:
    ...
    def fetch_bars(
        ...
        if cache_key in self._cache:
            ...
            if end_time:
                bars = self._filter_bars_by_time(bars, end_time)
            return bars
    ...
    def _filter_bars_by_time(self, bars: List[Dict], end_time: time) -> List[Dict]:
        """Filter bars to only include those before end_time on the trading date."""
        filtered = []
        for bar in bars:
            ts = bar.get('timestamp')
            if ts:
                bar_time = ts.time() if hasattr(ts, 'time') else time(ts.hour, ts.minute)
...
# =============================================================================
# STRUCTURE ANALYZER (Combines Calculator + Fetcher)
# =============================================================================
...
class StructureAnalyzer:
    ...
    def get_structure(
        ...
        """
        Get market structure for a specific timeframe at entry time.

        Args:
            ticker: Stock symbol
            timeframe: 'M5', 'M15', 'H1', or 'H4'
            trade_date: Trading date
            entry_time: Entry time (filter bars before this)

        Returns:
            StructureResult
        """
        bars = self.fetcher.fetch_bars(ticker, timeframe, trade_date, entry_time)
    ...
    def get_all_structures(
        ...
        for timeframe in ['M5', 'M15', 'H1', 'H4']:
            results[timeframe] = self.get_structure(ticker, timeframe, trade_date, entry_time)
