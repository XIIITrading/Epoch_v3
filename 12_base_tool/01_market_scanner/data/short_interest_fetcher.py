...
class ShortInterestFetcher:
    ...
    def load_all_short_data(self, force_refresh: bool = False) -> Dict[str, Dict]:
        """
        Load ALL available short interest data from Polygon.
        Used when scanning all US equities - kept for backward compatibility.
        """
        logger.warning("load_all_short_data is deprecated - use load_short_data_for_tickers instead")
        
        if not force_refresh and self.cache and self.cache_date:
            if datetime.now() - self.cache_date < timedelta(hours=1):
                logger.info(f"Using cached short interest data ({len(self.cache)} tickers)")
                return self.cache
        
        logger.info("Fetching ALL short interest data from Polygon...")
        ...
        try:
            for item in self.client.list_short_interest(
                ...
                if ticker not in all_data:
                    ...
                    if count % 500 == 0:
                        logger.info(f"Loaded {count} tickers with short interest data...")
            
            logger.info(f"Loaded short interest for {len(all_data)} tickers")
            
            # Update cache
            self.cache = all_data
            self.cache_date = datetime.now()
            self.cached_tickers = set(all_data.keys())
    ...
    def load_short_data_for_tickers(self, 
        ...
        This is a targeted fetch - only processes the exact tickers needed.
        ...
        if not force_refresh and self.cache and self.cache_date:
            # Cache is valid if:
            # 1. It's for the same date
            # 2. It contains all requested tickers
            # 3. It was fetched within the last hour
    ...
    def _calculate_short_percent(self, ticker: str, short_shares: float) -> float:
        ...
        For simplicity, using DTC * 3 as rough estimate.
        """
        if short_shares == 0:
            return 0
        ...
        if dtc > 0:
            ...
            # This is a simplification - ideally would use shares outstanding
