"""
Report generation for Candle Range indicator edge testing.
...
def generate_markdown_report(
    ...
    # Candle Range specific stats
    if 'candle_range_mean' in metadata:
        lines.append("**Candle Range Statistics:**")
        lines.append(f"- Mean: {metadata.get('candle_range_mean', 0):.3f}%")
        lines.append(f"- Median: {metadata.get('candle_range_median', 0):.3f}%")
        lines.append(f"- Std Dev: {metadata.get('candle_range_std', 0):.3f}%")
        lines.append("")

    # Data source note
    lines.append("**Data Source:** `m1_indicator_bars` table - using OHLC from prior M1 bar before entry")
    lines.append("")
    lines.append("**Candle Range Calculation:** `(high - low) / open * 100`")
    lines.append("")
    lines.append("---")
    lines.append("")
    ...
    # PyQt Tool Integration Notes
    lines.append("## PyQt Tool Integration")
    lines.append("")
    lines.append("### Standalone Candle Range Filters")
    lines.append("")
    lines.append("| Filter | Condition | Action | Expected WR |")
    lines.append("|--------|-----------|--------|-------------|")
    lines.append("| Large Range | Range >= 0.15% | +1 point | 51-53% |")
    lines.append("| Small Range | Range < 0.12% | -1 point (caution) | 37-38% |")
    lines.append("| Very Large Range | Range >= 0.20% | +2 points | 55%+ |")
    lines.append("")
    lines.append("### Composite Signal Filters (combine with Volume indicators)")
    lines.append("")
    lines.append("| Filter | Condition | Action | Expected WR |")
    lines.append("|--------|-----------|--------|-------------|")
    lines.append("| Momentum | High Vol + Range >= 0.15% | TAKE (Rejection) | 58-60% |")
    lines.append("| Absorption | High Vol + Range < 0.12% | SKIP | 35% |")
    lines.append("")
