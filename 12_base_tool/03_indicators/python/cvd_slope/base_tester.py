"""
Base infrastructure for CVD Slope indicator edge testing.
...
class EdgeTestResult:
    ...
    indicator: str              # e.g., "CVD Slope"
    test_name: str              # e.g., "CVD Slope Direction"
...
def fetch_cvd_slope_data(
    ...
    """
    Fetch trade data joined with CVD Slope from prior M1 bar.
    ...
    CVD Slope represents the trend/slope of Cumulative Volume Delta,
    indicating whether order flow momentum is increasing or decreasing.

    Returns DataFrame with columns:
        trade_id, date, ticker, model, direction, entry_price, entry_time,
        cvd_slope (from prior M1 bar), is_winner
    """
    query = """
    ...
    SELECT
        ...
        twpb.entry_time,
        twpb.prior_bar_time,
        -- CVD Slope from prior M1 bar
        m1.cvd_slope,
        -- Also include vol_delta for reference
        m1.vol_delta,
        m1.vol_roc,
