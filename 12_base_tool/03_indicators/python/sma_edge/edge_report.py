"""
Report generation for SMA indicator edge testing.
...
def generate_markdown_report(
    ...
    # Data source note
    lines.append("**Data Source:** `m1_indicator_bars` table - using SMA9, SMA21, sma_spread, and sma_momentum_label from prior M1 bar before entry")
    lines.append("")
    lines.append("---")
    lines.append("")

    # SMA Statistics
    if 'sma_spread_mean' in metadata:
        lines.append("## SMA Statistics")
        lines.append("")
        lines.append("| Metric | Value |")
        lines.append("|--------|-------|")
        lines.append(f"| SMA Spread Mean | {metadata.get('sma_spread_mean', 0):.4f} |")
        lines.append(f"| SMA Spread Std Dev | {metadata.get('sma_spread_std', 0):.4f} |")
        lines.append(f"| SMA Spread Min | {metadata.get('sma_spread_min', 0):.4f} |")
        lines.append(f"| SMA Spread Max | {metadata.get('sma_spread_max', 0):.4f} |")
        if 'bullish_pct' in metadata:
            lines.append(f"| Bullish Config (SMA9 > SMA21) | {metadata.get('bullish_pct', 0):.1f}% |")
            lines.append(f"| Bearish Config (SMA9 < SMA21) | {metadata.get('bearish_pct', 0):.1f}% |")
        lines.append("")
        lines.append("---")
        lines.append("")
