...
def _render_pre_trade_view(
    ...
    # Trade info panel at TOP (before chart)
    ...
    # DOW AI Pre-Trade Prediction (from batch analyzer)
    st.markdown("---")
    if supabase:
        try:
...
def _render_trade_info(trade: TradeWithMetrics):
...
def _render_post_trade_view(
    ...
    # DOW AI Post-Trade Review Section
    st.markdown("---")
    try:
