"""
Epoch Backtest Journal - Primary vs Secondary Module Runner
...
from prim_v_sec.analyzer import PrimarySecondaryAnalyzer
from prim_v_sec.report import PrimarySecondaryReport
...
def run(
    ...
    """
    Run Primary vs Secondary analysis and generate reports.
    ...
    if verbose and standalone:
        ...
        print("Primary vs Secondary Zone Analysis")
        print("=" * 60)
        print()
    ...
    if verbose:
        if start_date or end_date:
            print(f"[2/3] Analyzing trades from {start_date or 'start'} to {end_date or 'end'}...")
        else:
            print("[2/3] Analyzing all available trades...")

    analyzer = PrimarySecondaryAnalyzer(db)
    result = analyzer.analyze(start_date=start_date, end_date=end_date)

    if verbose:
        if result.model_stats:
            total_trades = result.overall_stats.get('total_trades', 0)
            print(f"  Analyzed {total_trades} trades across {result.total_days} days")
        else:
    ...
    report = PrimarySecondaryReport(result)
    ...
    if standalone:
        ...
        if verbose:
            print("  Added PDF page: Primary vs Secondary")

        if excel_exporter:
            report.export_to_excel(excel_exporter)
            if verbose:
                print("  Added Excel worksheet: Primary vs Secondary")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Epoch Primary vs Secondary Zone Analysis",
