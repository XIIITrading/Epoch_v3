...
class SupabaseClient:
    ...
    def fetch_trades(
        ...
        Args:
            ...
            ai_validated_only: If True, only return trades that have an ai_predictions record
        ...
        # Build FROM/JOIN clauses based on filters
        joins = []
        where_clauses = []

        if unreviewed_only:
            joins.append("LEFT JOIN trade_reviews r ON t.trade_id = r.trade_id")
            where_clauses.append("r.id IS NULL")

        if ai_validated_only:
            joins.append("INNER JOIN ai_predictions aip ON t.trade_id = aip.trade_id")

        join_sql = " ".join(joins)
        query = f"SELECT t.* FROM trades t {join_sql}"
    ...
    def fetch_trade_with_metrics(self, trade_id: str) -> Optional[TradeWithMetrics]:
        ...
        # Fetch canonical outcome from trades_m5_r_win (M5 ATR stop)
        canonical_query = """
            SELECT stop_price, stop_distance, max_r_achieved as r_achieved,
                   outcome, is_winner, pnl_r, outcome_method,
        ...
        try:
            with self.conn.cursor(cursor_factory=RealDictCursor) as cur:
                ...
                # Get canonical outcome from trades_m5_r_win
                cur.execute(canonical_query, (trade_id,))
    ...
    def fetch_trades_with_metrics(
        ...
        trades = self.fetch_trades(
            ...
            ai_validated_only=ai_validated_only,
            limit=limit
        )
        ...
        try:
            with self.conn.cursor(cursor_factory=RealDictCursor) as cur:
                ...
                # Fetch all canonical outcomes from trades_m5_r_win at once
                canonical_query = """
                    SELECT trade_id, stop_price, stop_distance,
                           max_r_achieved as r_achieved,
                           outcome, is_winner, pnl_r, outcome_method,
                           r1_price, r2_price, r3_price, r4_price, r5_price
                    FROM trades_m5_r_win
                    WHERE trade_id = ANY(%s)
                """
                cur.execute(canonical_query, (trade_ids,))
    ...
    def fetch_ai_prediction(self, trade_id: str) -> Optional[Dict[str, Any]]:
        ...
        query = """
            SELECT
                ...
                sma, h1_struct,
                snapshot,
                actual_outcome, actual_pnl_r, prediction_correct,
