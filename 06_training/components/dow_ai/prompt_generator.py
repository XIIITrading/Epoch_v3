...
RISK MANAGEMENT (Stop = M5 ATR 1.1x):
- Stop Price: ${stop_price:.2f}
...
def _format_rampup_section(ticker: str, trade_date: date, entry_time: time) -> str:
    ...
    lines.append("Time  │ Cndl% │ VolDelta │ VolROC │ SMA  │  M5 │ M15 │  H1")
    lines.append("──────┼───────┼──────────┼────────┼──────┼─────┼─────┼─────")
    ...
    for bar in bars:
        ...
        # M5 Structure
        m5_struct = bar.get('m5_structure')
        m5_str = "▲" if m5_struct == 'BULL' else "▼" if m5_struct == 'BEAR' else "-"

        # M15 Structure
        m15_struct = bar.get('m15_structure')
        m15_str = "▲" if m15_struct == 'BULL' else "▼" if m15_struct == 'BEAR' else "-"

        # H1 Structure
        h1_struct = bar.get('h1_structure')
        h1_str = "▲" if h1_struct == 'BULL' else "▼" if h1_struct == 'BEAR' else "-"

        lines.append(
            f"{time_str} │ {candle_str:>5} │ {delta_str:>8} │ {roc_str:>6} │ {sma_str:>4} │ {m5_str:>3} │ {m15_str:>3} │ {h1_str:>3}"
        )
    ...
    # Multi-timeframe fractal structure summary
    for tf_name, tf_col in [('M5', 'm5_structure'), ('M15', 'm15_structure'), ('H1', 'h1_structure')]:
        tf_bull = sum(1 for b in bars if b.get(tf_col) == 'BULL')
        tf_bear = sum(1 for b in bars if b.get(tf_col) == 'BEAR')
        tf_neut = len(bars) - tf_bull - tf_bear
        lines.append(f"- {tf_name} Structure: {tf_bull} BULL / {tf_bear} BEAR / {tf_neut} NEUTRAL")

    # Structure alignment at entry (last bar)
    if bars:
        last = bars[-1]
        m5_dir = last.get('m5_structure', 'NEUTRAL')
        m15_dir = last.get('m15_structure', 'NEUTRAL')
        h1_dir = last.get('h1_structure', 'NEUTRAL')
        aligned = m5_dir == m15_dir == h1_dir and m5_dir in ('BULL', 'BEAR')
        alignment_str = f"ALIGNED {m5_dir}" if aligned else f"M5={m5_dir} / M15={m15_dir} / H1={h1_dir}"
        lines.append(f"- Structure at Entry: {alignment_str}")
