...
INDICATOR_LABELS = [
    ...
    'M5 Struct',   # m5_structure - format: ▲/▼/─
    'M15 Struct',  # m15_structure - format: ▲/▼/─
    'H1 Struct',   # h1_structure - format: ▲/▼/─
]
    ...
    for indicator_idx, indicator_name in enumerate(INDICATOR_LABELS):
        ...
        for bar_idx, (_, row) in enumerate(df.iterrows()):
            ...
            if indicator_name == 'Candle %':
                ...
                value = format_structure(row.get('m5_structure'))
                text_color = get_structure_color(row.get('m5_structure'))
            elif indicator_name == 'M15 Struct':
                value = format_structure(row.get('m15_structure'))
                text_color = get_structure_color(row.get('m15_structure'))
...
# =============================================================================
# CANDLESTICK-ONLY CHART (for PyQt split view)
# =============================================================================

def build_rampup_candlestick_only(
    df: pd.DataFrame,
    trade_date: date,
    entry_time: time,
    direction: str = 'LONG'
    ...
    """
    Build ramp-up chart with ONLY the candlestick portion (no indicator table).
    The indicator table is rendered as a separate PyQt QTableWidget.

    Args:
        df: DataFrame with M1 bars
        trade_date: Trading date
        entry_time: Entry time
        direction: Trade direction

    Returns:
        Plotly Figure with candlestick + SMA/VWAP overlays
    """
    if df.empty:
        fig = go.Figure()
        fig.add_annotation(
    ...
    x_vals = list(range(num_bars))
    x_labels = [
        row['bar_time'].strftime('%H:%M') if hasattr(row['bar_time'], 'strftime')
        else str(row['bar_time'])[:5]
    ...
    entry_str = entry_time.strftime('%H:%M') if hasattr(entry_time, 'strftime') else str(entry_time)

    fig.update_layout(
        title=dict(
            text=f"M1 Ramp-Up | Entry: {entry_str}",
            font=dict(size=13, color=CHART_CONFIG['text_color']),
    ...
    fig.update_xaxes(
        ...
        gridcolor=CHART_CONFIG['grid_color'],
        tickmode='array',
        tickvals=x_vals[::5],  # Show every 5th label
...
def render_rampup_chart(
    ...
    """
    Render the complete ramp-up chart for a trade (original Plotly version).
...
def render_rampup_split(
    ...
    """
    Render ramp-up as split components: candlestick figure + indicator DataFrame.
    Used by PyQt UI where the indicator table is a native QTableWidget.
    ...
    Returns:
        (fig, df) - Plotly Figure (candlestick only) and DataFrame with indicator data.
        fig is None if no data.
    """
    df = fetch_m1_bars_for_rampup(ticker, trade_date, entry_time, RAMPUP_BARS)
    ...
    fig = build_rampup_candlestick_only(df, trade_date, entry_time, direction)
    return fig, df
