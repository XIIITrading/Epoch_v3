...
@st.cache_data(ttl=600)
def load_trades_m5_r_win(date_from, date_to):
    """
    Load canonical trade outcomes from trades_m5_r_win with caching.
    ...
    """
    client = get_client()
    return client.fetch_trades_m5_r_win(date_from=date_from, date_to=date_to)
...
def main():
    ...
    with tab_metrics:
        ...
        # Load canonical outcomes from trades_m5_r_win for summary cards and Monte AI
        unified_outcomes = load_trades_m5_r_win(date_from=date_from, date_to=date_to)

        # Calculate overall_stats from canonical unified outcomes
        if unified_outcomes:
            _uo_df = pd.DataFrame(unified_outcomes)
            _uo_total = len(_uo_df)
            _uo_wins = int(_uo_df['is_winner'].sum())
            _uo_losses = _uo_total - _uo_wins
            _uo_win_rate = (_uo_wins / _uo_total * 100) if _uo_total > 0 else 0
            _uo_avg_r = float(_uo_df['pnl_r'].mean()) if 'pnl_r' in _uo_df.columns else 0
            _uo_total_r = float(_uo_df['pnl_r'].sum()) if 'pnl_r' in _uo_df.columns else 0
            overall_stats = {
                "total": _uo_total,
                "wins": _uo_wins,
                "losses": _uo_losses,
                "win_rate": round(_uo_win_rate, 1),
                "avg_points": round(_uo_avg_r, 2),
                "total_points": round(_uo_total_r, 2),
                "median_mfe_pct": 0,
                "median_mae_pct": 0
            }
        else:
            # Fallback to mfe_mae_potential if trades_m5_r_win not available
        ...
        # =================================================================
        # IMPORTANT: Monte AI prompt always uses DEFAULT stop type (M5 ATR 1.1x Close)
        # for consistency and reproducibility, regardless of UI selection above.
        # This aligns with trades_m5_r_win canonical outcomes.

        # Info about Monte AI using default stop
        st.info(
            "**Monte AI Prompt Note:** The generated prompt always uses the default stop type "
            "(M5 ATR 1.1x Close) for consistency, regardless of your selection above. "
