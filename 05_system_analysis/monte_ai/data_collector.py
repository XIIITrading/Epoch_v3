...
def _format_stop_analysis(stop_analysis: Dict[str, Any], use_default: bool = True) -> str:
    ...
    Args:
        ...
        use_default: If True (default), always format using m5_atr data.
    ...
    # Default stop type configuration (canonical: M5 ATR aligned with trades_m5_r_win)
    DEFAULT_STOP_TYPE = 'm5_atr'
    DEFAULT_STOP_NAME = 'M5 ATR (1.1x Close)'
    ...
    lines.append("    M5 ATR (Close): Volatility-based, triggers on M5 close (DEFAULT - Canonical)")
...
def _format_model_stats_with_stop(stop_analysis: Dict[str, Any]) -> str:
    ...
    Always uses default stop type (m5_atr) for consistency.
    ...
    results = stop_analysis.get('results', {})
    default_outcomes = results.get('m5_atr', [])
    ...
    lines = [
        "WIN RATE BY MODEL (Using: M5 ATR 1.1x Close):",
        "-" * 60,
